...
The `getClient()` function gives you access to the full [`@commet/node` SDK](https://commet.co/docs/sdks/node?utm_source=xmcp&utm_medium=docs&utm_campaign=plugin), allowing you to leverage all Commet features in your MCP tools. The `getCustomerId()` function returns the customer ID extracted from the request header.
